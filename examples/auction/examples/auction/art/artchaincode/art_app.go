...
type InsureObject struct {
	InsureID         string
	RecType          string
	BuyerName        string
	Industry         string
	InsuranceType    string
	CompanyDetail    string // Could included details such as who created the Art work if item is a Painting
	InsureDetail     string
	InsuranceAmount  string
	MaxPremium       string
	AES_Key          []byte // This is generated by the AES Algorithms
	CurrentInsurerID string // This is validated for a user registered record
...
////////////////////////////////////////////////////////////////////////////////
type InsureLog struct {
	InsureID       string // PRIMARY KEY
	RecType        string
...
type AuctionRequest struct {
	...
	MaxPremium    string // reserver price > previous purchase price
...
func PostItem(stub shim.ChaincodeStubInterface, function string, args []string) pb.Response {
	...
	itemObject, err := CreateInsureObject(args[0:])
	if err != nil {
	...
	if err != nil {
		...
		response := PostInsureLog(stub, itemObject, "INITIAL", "DEFAULT", args[12])
		if response.Status != shim.OK {
			fmt.Println("PostInsureLog() : write error while inserting record\n")
			return shim.Error("PostInsureLog() : write error while inserting record : Error : " + err.Error())
		}
...
func CreateInsureObject(args []string) (InsureObject, error) {

	var err error
	var myItem InsureObject

	// Check there are 13 Arguments provided as per the the struct - two are computed
	if len(args) != 13 {
		fmt.Println("CreateInsureObject(): Incorrect number of arguments. Expecting 13 ")
		return myItem, errors.New("CreateInsureObject(): Incorrect number of arguments. Expecting 13 ")
	}
	...
	_, err = strconv.Atoi(args[0])
	if err != nil {
		fmt.Println("CreateInsureObject(): ART ID should be an integer create failed! ")
		return myItem, errors.New("CreateInsureObject(): ART ID should be an integer create failed!")
	}
	...
	if _, err := os.Stat(imagePath); err == nil {
		...
		fmt.Println("CreateInsureObject(): Cannot find or load Picture File = %s :  %s\n", imagePath, err)
		return myItem, errors.New("CreateInsureObject(): ART Picture File not found " + imagePath)
	}
	...
	myItem = InsureObject{args[0], args[1], args[2], args[3], args[4], args[5], args[6], args[7], args[8], args[9], AES_enc, AES_key, fileType, args[10], args[11], args[12]}

	fmt.Println("CreateInsureObject(): Item Object created: ", myItem.ItemID, myItem.AES_Key)
...
func UpdateInsureObject(stub shim.ChaincodeStubInterface, ar []byte, hammerPrice string, buyer string) pb.Response {

	var err error
	myItem, err := JSONtoAR(ar)
	if err != nil {
		fmt.Println("UpdateInsureObject() : Failed to create Art Record Object from JSON ")
		return shim.Error("UpdateInsureObject() : Failed to create Art Record Object from JSON : Error : " + err.Error())
	}
	...
	if err != nil {
		fmt.Println("UpdateInsureObject() : Failed ReplaceObject in ItemTable into Blockchain ")
		return shim.Error("UpdateInsureObject() : Failed ReplaceObject in ItemTable into Blockchain : Error : " + err.Error())
	}
	fmt.Println("UpdateInsureObject() : ReplaceObject in Item successful ")
	...
	if err != nil {
		fmt.Println("UpdateInsureObject() : Failed ReplaceObject in ItemCategory into Blockchain ")
		return shim.Error("UpdateInsureObject() : Failed ReplaceObject in ItemCategory into Blockchain : Error : " + err.Error())
	}

	fmt.Println("UpdateInsureObject() : ReplaceObject in ItemCategory successful ")
...
func TransferItem(stub shim.ChaincodeStubInterface, function string, args []string) pb.Response {
	...
	response = PostInsureLog(stub, myItem, "Transfer", args[1], args[5])
	if response.Status != shim.OK {
		fmt.Println("TransferItem() : PostInsureLog() write error while inserting record\n")
		return shim.Error(err.Error())
	}
...
// Valid Status for InsureLog =  OnAuc, OnSale, NA, INITIAL
// Valid AuctionedBy: This value is set to "DEFAULT" but when it is put on auction Auction House ID is assigned
// PostInsureLog IS NOT A PUBLIC API and is invoked every time some event happens in the Item's life
// The currentDateTime must be provided by Client
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////

func PostInsureLog(stub shim.ChaincodeStubInterface, item InsureObject, status string, ah string, currentDateTime string) pb.Response {

	iLog := ItemToInsureLog(item, currentDateTime)
	iLog.Status = status
	iLog.AuctionedBy = ah

	buff, err := InsureLogtoJSON(iLog)
	if err != nil {
		fmt.Println("PostInsureLog() : Failed Cannot create object buffer for write : ", item.ItemID)
		return shim.Error("PostInsureLog(): Failed Cannot create object buffer for write : " + item.ItemID)
	} else {
		// Update the ledger with the Buffer Data
...
func PostAuctionRequest(stub shim.ChaincodeStubInterface, function string, args []string) pb.Response {
	...
	if err != nil {
		...
		response := PostInsureLog(stub, io, "ReadyForAuc", ar.AuctionHouseID, ar.TimeStamp)
		if response.Status != shim.OK {
			fmt.Println("PostInsureLog() : write error while inserting record\n")
			return shim.Error(err.Error())
		}
...
func PostTransaction(stub shim.ChaincodeStubInterface, function string, args []string) pb.Response {
	...
	response = UpdateInsureObject(stub, lastUpdatedItemOBCObject, ar.HammerPrice, ar.UserId)
	...
	response = PostInsureLog(stub, itemObject, "NA", "DEFAULT", args[5])
	if response.Status != shim.OK {
...
//////////////////////////////////////////////////////////
func JSONtoAR(data []byte) (InsureObject, error) {
...
//////////////////////////////////////////////////////////
func ARtoJSON(ar InsureObject) ([]byte, error) {
...
//////////////////////////////////////////////////////////
func InsureLogtoJSON(item InsureLog) ([]byte, error) {
...
//////////////////////////////////////////////////////////
func JSONtoInsureLog(ithis []byte) (InsureLog, error) {
...
//////////////////////////////////////////////
// Create an InsureLog from Item
//////////////////////////////////////////////

func ItemToInsureLog(io InsureObject, cdt string) InsureLog {
...
// ./peer chaincode query -l golang -n mycc -c '{"Function": "GetInsureLog", "Args": ["1000"]}'
////////////////////////////////////////////////////////////////////////////
func GetInsureLog(stub shim.ChaincodeStubInterface, function string, args []string) pb.Response {

	// Check there are 1 Arguments provided as per the the struct - two are computed
	// See example
	if len(args) < 1 {
		fmt.Println("GetInsureLog(): Incorrect number of arguments. Expecting 1 ")
		fmt.Println("GetInsureLog(): ./peer chaincode query -l golang -n mycc -c '{\"Function\": \"GetItem\", \"Args\": [\"1111\"]}'")
		return shim.Error("CreateInsureObject(): Incorrect number of arguments. Expecting 12 ")
	}

	rs, err := itpUtils.GetList(stub, "ItemHistory", args)
	if err != nil {
		error_str := fmt.Sprintf("GetInsureLog operation failed. Error marshaling JSON: %s", err)
		return shim.Error(error_str)
	}
	...
	// Iterate through result set
	var i int
	var tlist []InsureLog // Define a list
	for i = 0; rs.HasNext(); i++ {
		...
		il, err := JSONtoInsureLog(myKeyVal)
		if err != nil {
			error_str := fmt.Sprintf("GetInsureLog() operation failed - Unmarshall Error. %s", err)
			fmt.Println(error_str)
			return shim.Error(error_str)
		}
		fmt.Println("GetInsureLog() : my Value : ", il)
...
func GetItemListByCat(stub shim.ChaincodeStubInterface, function string, args []string) pb.Response {
	...
	if len(args) < 1 {
		...
		return shim.Error("CreateInsureObject(): Incorrect number of arguments. Expecting 1 ")
	}
	...
	// Iterate through result set
	var i int
	var tlist []InsureObject // Define a list
	for i = 0; rs.HasNext(); i++ {
